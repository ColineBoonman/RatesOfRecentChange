...
if(length(files)!=length(unlist(unique_chunks))){
    f1 = future(get_occ_cell_indices_loop(sp,unique_chunks[[1]],resDir,out_raster,kg2,colselect)) %plan% multicore
    f2 = future(get_occ_cell_indices_loop(sp,unique_chunks[[2]],resDir,out_raster,kg2,colselect)) %plan% multicore
    f3 = future(get_occ_cell_indices_loop(sp,unique_chunks[[3]],resDir,out_raster,kg2,colselect)) %plan% multicore
    f4 = future(get_occ_cell_indices_loop(sp,unique_chunks[[4]],resDir,out_raster,kg2,colselect)) %plan% multicore
    f5 = future(get_occ_cell_indices_loop(sp,unique_chunks[[5]],resDir,out_raster,kg2,colselect)) %plan% multicore
    f6 = future(get_occ_cell_indices_loop(sp,unique_chunks[[6]],resDir,out_raster,kg2,colselect)) %plan% multicore
    f7 = future(get_occ_cell_indices_loop(sp,unique_chunks[[7]],resDir,out_raster,kg2,colselect)) %plan% multicore
    f8 = future(get_occ_cell_indices_loop(sp,unique_chunks[[8]],resDir,out_raster,kg2,colselect)) %plan% multicore
    value(f1)
    value(f2)
    value(f3)
    value(f4)
