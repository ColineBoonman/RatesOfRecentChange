make.plots.bar.and.smoothline = function(window,data,eoo,FigureDir){
  ...
  df=subset(data,select=c(IUCNstatus,EOO)) 
  df$IUCNstatus = as.character(df$IUCNstatus)
  df$IUCNstatus[which(is.na(df$IUCNstatus))]=as.character("Not Evaluated")
  df$IUCNstatus = as.factor(as.character(df$IUCNstatus))
  ...
  for(i in 1:length(idx)){
    ...
    colnames(d)[3]="rate"
    if(eoo=="all") d=d
    if(eoo=="small") d=d[d$EOO< 5000,]
    if(eoo=="medium") d=d[d$EOO>5000 & d$EOO<= 20000,]
    if(eoo=="large") d=d[d$EOO>= 20000,]
    ...
    # grey orange blue = "#999999", "#E69F00", "#56B4E9"
    # green orange purple = 1b9e77 d95f02 7570b3
    # yellow red blue = FFFF00 FF0000 0000FF
    # grey red blue = 999999 FF0000 0000FF
    # lightyellow black orange = fed98e 000000 d95f0e #used for fill
    # yellow black brown = fe9929 000000 993404
    # green red yellow = 1b9e77 FF0000 FFFF00
    
    n1 = d[which(d$IUCNstatus=="Not Threatened"),]
    n2 = d[which(d$IUCNstatus=="Threatened"),]
    n3 = d[which(d$IUCNstatus=="Vulnerable"),]
    n4 = d[which(d$IUCNstatus=="DD"),]
    n5 = d[which(d$IUCNstatus=="Not Evaluated"),]
    ...
    #geom_segment(aes(x = extreme.low1, y = 1, xend = extreme.low1, yend = 1.5),col="black", linetype=2, lwd=1)+
      
    # if("slope" %in% colnames(data)[idx[i]] & "fire" %!in% colnames(data)[idx[i]]){
    #   if(min(d$rate,na.rm=T)<0) p=p + annotate(geom="text", x=z1$rate[which(z1$rate==0)-2], y=maxy/2, label=paste0(MIN,"%"),color="black",cex=6, hjust = "right")
    #   p=p + annotate(geom="text", x=z1$rate[which(z1$rate==0)+2], y=maxy/2, label=paste0(MAX,"%"),color="black",cex=6, hjust = "left")
    # }
...
make.plots.ordered = function(window,data,eoo,FigureDir){
  if(window=="full"){idx=c(grep("slope_vpd",colnames(data)),
                           grep("slope_prec",colnames(data)),
                           grep("slope_tmin",colnames(data)),
                           grep("slope_tmax",colnames(data)),
  ...
  for(i in 1:length(idx)){
    ...
    summary(d$rate)
    if(max(d$rate,na.rm=T)>16) d$rate = round(d$rate/2)*2 
    if(max(d$rate,na.rm=T)<=16  & max(d$rate,na.rm=T)>7) d$rate = round(d$rate)
    if(max(d$rate,na.rm=T)<=7 & max(d$rate,na.rm=T)>2.5) d$rate = round(d$rate/0.5)*0.5
    if(max(d$rate,na.rm=T)<=2.5 & max(d$rate,na.rm=T)>1.5 & min(d$rate,na.rm=T)<0) d$rate = round(d$rate/0.25)*0.25
    if(max(d$rate,na.rm=T)<=2.5 & max(d$rate,na.rm=T)>1.5 & min(d$rate,na.rm=T)==0) d$rate = round(d$rate/0.1)*0.1
    if(max(d$rate,na.rm=T)<=1.5 & max(d$rate,na.rm=T)>0.5) d$rate = round(d$rate/0.1)*0.1
    if(max(d$rate,na.rm=T)<=0.5 & max(d$rate,na.rm=T)>0.35) d$rate = round(d$rate/0.05)*0.05
    if(max(d$rate,na.rm=T)<=0.35 ) d$rate = round(d$rate/0.02)*0.02
    ...
    p = 
      ...
      ggplot(d, aes(x = rate)) +
      stat_ecdf(aes(color = IUCNstatus), geom = "step", size = 1.5) +
      stat_ecdf(color="black", geom = "step", size = 1.5) +
      labs(title="",x=colnames(data)[idx[i]], y = "Number of Species")+
      theme_linedraw()
    
    ggsave(p, file=paste0(FigureDir,"RecentChangeImages/ordered/Recent.Change.",colnames(data)[idx[i]],"__EOO",eoo,"__Window",window,".pdf"),
           width = 6, height = 9)
  }
}
...
make.complete = function(FigureDir,dir="RecentChangeImages/bar.and.smoothline"){
  ...
  img.all <-c(cropland,tcchange_area,urban,hGFC,
  ...
  img.all <-c(allcropland,alltcchange_area,allurban,allhGFC,allfire,
              largecropland,largetcchange_area,largeurban,largehGFC,largefire,
              mediumcropland,mediumtcchange_area,mediumurban,mediumhGFC,mediumfire,
              smallcropland,smalltcchange_area,smallurban,smallhGFC,smallfire,
              legend)
